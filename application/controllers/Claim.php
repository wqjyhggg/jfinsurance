...
class Claim extends MY_Controller {
	...
	public function index()
	{
		...
		if ($this->input->post('submit')) {
			...
			$para = array();
			$para['product_short'] = $this->input->post('product_short');
			$para['policy_number'] = $this->input->post('policy_number');
			...
			$para['claim_date2'] = $this->input->post('claim_date2');
	...
	public function add($customer_id) {
		...
		if (!empty($customer_id)) {
			...
			if ($customer) {
				$plan = $this->customer_model->get_plan_by_id($customer['plan_id']);
				if ($plan && ($plan['status_id'] > 2)) {
					...
					$para['lastname'] = $customer['customer_id'];
	...
	public function form($claim) {
		...
		if ($this->input->post('claim_id')) {
			...
			$this->data['claim_id'] = $plan['claim_id'];
		} else {
		...
		if ($this->input->post('customer_id')) {
			...
			$this->data['customer_id'] = $plan['customer_id'];
		} else {
			$this->data['error_message'] = 'Unknown Customer';
		}
		...
		if ($this->input->post('product_short')) {
			...
			$this->data['product_short'] = $plan['product_short'];
		} else {
			$this->data['error_message'] = 'Unknown Product';
		}
		if ($this->input->post('policy_number')) {
			...
			$this->data['policy_number'] = $plan['policy_number'];
		} else {
			$this->data['error_message'] = 'Unknown Policy';
		}
		if ($this->input->post('cliaim_number')) {
			...
			$this->data['cliaim_number'] = $plan['cliaim_number'];
		} else {
		...
		if ($this->input->post('lastname')) {
			...
			$this->data['lastname'] = $plan['lastname'];
		} else {
			$this->data['lastname'] = '';
		}
		if ($this->input->post('firstname')) {
			...
			$this->data['firstname'] = $plan['firstname'];
		} else {
			$this->data['firstname'] = '';
		}
		if ($this->input->post('birthday')) {
			...
			$this->data['birthday'] = $plan['birthday'];
		} else {
			$this->data['birthday'] = '';
		}
		...
		if ($this->input->post('claim_date')) {
			...
			$this->data['claim_date'] = $plan['claim_date'];
		} else {
		...
		if ($this->input->post('claimed')) {
			...
			$this->data['claimed'] = $plan['claimed'];
		} else {
			$this->data['claimed'] = 0;
		}
		...
		if ($this->input->post('cheque_number')) {
			...
			$this->data['cheque_number'] = $plan['cheque_number'];
		} else {
		...
		if ($this->input->post('coverage_code_id')) {
			...
			$this->data['coverage_code_id'] = $plan['coverage_code_id'];
		} else {
		...
		if ($this->input->post('service_date')) {
			...
			$this->data['service_date'] = $plan['service_date'];
		} else {
		...
		if ($this->input->post('diagnosis')) {
			...
			$this->data['diagnosis'] = $plan['diagnosis'];
		} else {
			$this->data['diagnosis'] = '';
		}
		...
		if ($this->input->post('decline_reason')) {
			...
			$this->data['decline_reason'] = $plan['decline_reason'];
		} else {
