...
//		print_r(get_class_methods(get_class($objPHPExcel)));
  ...
  public function gerenate_pdf()
    ...
    $sql = "SELECT plan_id, count(plan_id) as cnt FROM `plan_history` GROUP by plan_id HAVING cnt>2";
    $rt = $this->db->query($sql)->result_array();
    foreach ($rt as $rc) {
      $sql = "UPDATE `plan_history` ph SET ph.add_time=(SELECT added FROM payment pa WHERE ph.payment_id=pa.payment_id) WHERE plan_id=".$rc["plan_id"]." AND ishead=1";
      $this->db->query($sql);
    }
	}
}
