...
class Premium3 extends MY_Controller
{
  /**
  ...
  public function index()
  {
    ...
    $data['payment_added_from'] = empty($this->input->post('payment_added_from'))?"":$this->input->post('payment_added_from');
    $data['payment_added_to'] = empty($this->input->post('payment_added_to'))?"":$this->input->post('payment_added_to');
    ...
    if ($this->input->post('export')) {
      ...
      if ($para_data['payment_added_from'] || $para_data['payment_added_to']) {
        $this->backrun_model->add_run(Backrun_model::ORMonthlyPremium, json_encode($para_data));
      }
    ...
    $data['download_request'] = $this->backrun_model->get_job_list(Backrun_model::ORMonthlyPremium);
    $data['product_list'] = $this->product_model->get_available_product_list();

    $this->load->common('reports/premium3', $data);
  }
  ...
  public function report()
  {
    ...
    $data["run_type"] = Backrun_model::ORMonthlyPremium;

    $this->backrun_model->ORMonthlyPremium(0, $data);
  }
}
