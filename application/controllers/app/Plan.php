...
class Plan extends CI_Controller
{
  ...
  public function update()
  {
    ...
    $old_claim_flag = $data["plan"]['claim_flag'];
    $new_claim_flag = 0;
    ...
    if (($old_claim_flag == 2) && ($new_claim_flag == 0)) {
      // Unblocked
      $this->plan_model->update($plan_id, array('claim_flag' => 0), array(), $user);
        $para = array(
          'plan_id' => $plan_id,
          'customer_id' => $plan['customer_id'],
          'payment_id' => 0,
    ...
    $data = array();
      ...
      return $this->app_model->return_error("Can't refund plan");
        ...
        // Add history
			...
			$note = "Api Refund at " . $dt['added'] . " amount: " . $refund_amount . " admin fee: " . ($admin_fee + $added_admin_fee) . "; " . $plan['note'];
			...
			if ($id = $this->plan_history_model->add($plan_id, Plan_model::REFUND)) {
				if (!empty($plan["monthlypay"])) {
					$this->plan_history_model->update($id, array("payment_id" => $payment_id, "premium" => ($total_amount - $added_admin_fee - $refund_amount), "expiry_date" => $refund_date, "totaldays" => $totaldays, "note" => "Api Refunded Recode"));
				} else {
      ...
      return $this->app_model->return_error("Can't cancel plan");
        ...
        if (empty($exnote)) {
          $exnote = $this->input->post('reason');
				...
				$note = "Api Reason: " . $exnote . ", cancel at " . $dt['added'] . " amount: " . $refund_amount . " admin fee: " . ($admin_fee + $added_admin_fee) . "; " . $plan['note'];
  ...
  public function sendquote() {
    ...
    $data = $this->input->post();
		if (empty($data["premium"])) {
			return $this->app_model->return_error("Missing Premium");
		}
		if (empty($data["emailaddr"]) || !$this->verify_model->isEmail($data['emailaddr'])) {
			return $this->app_model->return_error("Missing Email Address");
		}
