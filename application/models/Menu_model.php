	...
	public function load_meun() {
		$meunArr = array(
			...
			1 => array(
					...
					"<a href='" . base_url('notify') . "' class='leftmeun'><i class='fa fa-cog'></i>" . $this->lang->line('Notify Me') . "</a>",
...
*/
      ...
      106 =>array(
    	...
    	$menu = $meunArr[$group];
    	if ($beuser && ($beuser['user_id'] == 2202)) {
    		unset($menu[2]);
    	}
    	if ($user && ($user['user_group_id'] != 105) && ($user['user_group_id'] != 103) && ($user['user_group_id'] != 3 )) {
    		if (($user['user_id'] == 2654) || ($user['user_id'] == 2661) || ($user['user_id'] == 2762) || ($user['user_id'] == 1)) {
