...
class Plan_model extends CI_Model {
  ...
  public function get_plan_by_policy($policy) {
	...
	public function get_today_plan_counts($user_id) {
		$sql = "SELECT count(*) as count FROM plan WHERE user_id='" . intval($user_id) . "' AND apply_date >= CURDATE()";
    ...
    $update_history = 0;
      ...
      if (($plan['status_id'] == self::PAID) || ($plan['status_id'] == self::SOLD)) {
			// ((($plan['status_id'] == self::PAID) || ($plan['status_id'] == self::SOLD)) && (($plan["monthlypay"] != 1) || $premiumchanged))
    ...
    $users = [];
      ...
      foreach ($rows as $row) {
		...
		if (!empty($para['status_ids'])) {
			$where[] = "p.status_id in (" . $this->db->escape($para['status_ids']) . ")";
		}
    if (!empty($users)) {
