...
class Report_model extends CI_Model
{
  ...
  public function get_sales_report_jf($para)
  {
    ...
    $sql .= "		CONCAT(cu.firstname, ' ', cu.lastname) as insured,";
    $sql .= "		CASE";
		$sql .= "		  WHEN (SELECT m.pay_type FROM monthly_payment m WHERE m.payment_id = pa.payment_id) IS NULL THEN 'premium'";
		$sql .= "		  WHEN (SELECT m.pay_type FROM monthly_payment m WHERE m.payment_id = pa.payment_id) = 0 THEN 'Initial'";
		$sql .= "		  ELSE 'recurring'";
  ...
  public function get_sales_report_insurer($para)
  {
    ...
    $sql .= "	IF(pa.admin_fee < 0, pa.amount + pa.admin_fee, pa.amount - pa.admin_fee) AS policy_premium,";   // 2026-09-07 change it back
  ...
  public function get_commission_report($para)
  {
    ...
    $sql .= "	pl.expiry_date,";
    $sql .= "	pl.totaldays AS total_days,";
    $sql .= "	IF(pa.admin_fee < 0, pa.amount + pa.admin_fee, pa.amount - pa.admin_fee) AS premium,";  // 2026-09-07 change it back
